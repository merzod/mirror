...
class TimerProcessor(Processor):
	...
	def processCommandByMyself(self, cmd):
		...
		if t is None:
			...
			if canceled is None:
				logging.info('Timer not run. Last was scheduled for %d sec, and finished %d sec ago' % (t.interval, time.time() - started - t.interval))
				Voice.getInstance().say('Таймер на %s завершился %s назад' % (secToString(t.interval), secToString(time.time() - started - t.interval)))
			else:
				logging.info('Timer not run. Last was scheduled for %d sec, and canceled %d sec ago' % (t.interval, time.time() - canceled))
				Voice.getInstance().say('Таймер на %s был отменен %s назад' % (secToString(t.interval), secToString(time.time() - canceled)))
